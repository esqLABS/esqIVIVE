...
IVIVE_MM<-function(typeValue,units,typeSystem,fu_invitro,Vmax=NULL,Km_uM=NULL,expData=NULL,tissue=NULL,species=NULL,volMedium=NULL,
  ...
  # check if the arguments are valid
  rlang::arg_match(typeValue, c("exp_curve",
                                 "Vmax&Km"))
...
get_MM<-function(mm_exp_curve_xy){
  library(ggplot2)
    ...
    plot_diagnosis<-ggplot(mm_exp_curve,aes(x=Concentration_uM,y=Velocity_nmol.min.mg.mic.protein))+
      geom_point()+
      stat_function(fun = function(x) mm_fuction(x), colour = "blue")


    #Add fit values in dataframe for calculations
    fitresults_vmax_km=data.frame("Mean"=c(coefficients(fitmm )["Km"],
                                           coefficients(fitmm )["Vmax"]),
