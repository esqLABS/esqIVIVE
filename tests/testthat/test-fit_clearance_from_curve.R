test_that("fit_clearance_from_curve fits the expected kcat from the clearance.csv fixture", {
  ...
  expect_equal(unname(kcat["Mean_kcat_min-1"]), 0.0187109695257764, tolerance = 1e-6)
  expect_equal(unname(kcat["2.5%_CI_kcat"]), 0.0173403013706227, tolerance = 1e-6)
  expect_equal(unname(kcat["95%_CI_kcat"]), 0.0202073107873403, tolerance = 1e-6)
})
