...
#'
#' @description
#' collection of QSAR to obtain affintiy to the different components in serum, membrane lipids (memlip)
#' albumin (alb) and globulin (glob)
#' 
#' @param QSAR type of QSAR, it can be logP based or PPLFER based ( still sorting the ionization)
#' @param logP is the lipophlicity as given b logKow
...
#'QSARs_plasma(QSAR="PPLFER",logP=2,pKa=c(3,0),ionization=c("acid",0),LFER_E=1,LFER_B=0,LFER_A=1.5,LFER_S=0.8,LFER_V=2)
 ...
 QSARs_plasma<-function(QSAR,logP,pKa,ionization,LFER_E=NULL,LFER_B=NULL,LFER_A=NULL,LFER_S=NULL,LFER_V=NULL){
  ...
  if (QSAR=="logP"){
    ...
    kbsa_Lkg=0.85+0.63*LFER_Ei-0.63*LFER_Si-0.05*LFER_Ai+2.08*LFER_Bi+2.06*LFER_Vi+3.13*LFER_J
    kalb_Lkg=kbsa_Lkg
    kmus_Lkg=-0.24+0.68*LFER_E-0.76*LFER_S-2.29*LFER_B+2.51*LFER_V
    kglob_Lkg=kmus_Lkg
    
  }
  return(c("kmemlip_LL"=kmemlip_LL,"kalb_Lkg"= kalb_Lkg,
           "kglob_Lkg"=kglob_Lkg))
